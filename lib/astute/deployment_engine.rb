      ...
      attrs['nodes'] = nodes.map do |n|
        {
          ...
          'name'                 => n['fqdn'].split(/\./)[0],
          'role'                 => n['role'],
          'internal_address'     => n['network_data'].select {|nd| select_ifaces(nd['name'], 'admin')}[0]['ip'].split(/\//)[0],
          'internal_br'          => n['internal_br'],
          'internal_netmask'     => n['network_data'].select {|nd| select_ifaces(nd['name'], 'admin')}[0]['netmask'],
          'storage_address'      => n['network_data'].select {|nd| select_ifaces(nd['name'], 'admin')}[0]['ip'].split(/\//)[0],
          'storage_netmask'      => n['network_data'].select {|nd| select_ifaces(nd['name'], 'admin')}[0]['netmask'],
